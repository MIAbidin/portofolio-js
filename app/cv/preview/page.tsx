...
export default function CVPreviewPage() {
  ...
  // ── Derive owner + experiences from raw data + current lang ────────────
    ...
    <>
      <style jsx global>{`
        ...
        .lang-btn.active-en {
          ...
          color: #00d9ff;
          box-shadow: 0 0 12px rgba(0,217,255,0.2), inset 0 1px 0 rgba(0,217,255,0.2);
        }
        .lang-btn.active-id {
          background: rgba(167,139,250,0.15);
          color: #a78bfa;
          box-shadow: 0 0 12px rgba(167,139,250,0.2), inset 0 1px 0 rgba(167,139,250,0.2);
        }
        .lang-btn.inactive {
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0a0e27 0%, #111633 50%, #0a0e27 100%)', paddingBottom: 80 }}>
        ...
        <div
          ...
          style={{
            ...
            background: 'rgba(9,13,35,0.96)', backdropFilter: 'blur(12px)',
        ...
        >
          <div style={{ maxWidth: '210mm', margin: '0 auto', padding: '0 2rem', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <button
              onClick={() => router.back()}
              style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', color: '#475569', fontFamily: 'DM Mono, monospace', fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', padding: 0, transition: 'color 0.2s' }}
            ...
            <div className="lang-switcher">
              <button
                className={`lang-btn ${lang === 'en' ? 'active-en' : 'inactive'}`}
                onClick={() => handleLangSwitch('en')}
              >
                <span style={{ fontSize: 15 }}>🇬🇧</span>
                EN
                {lang === 'en' && (
                  <span style={{ width: 5, height: 5, borderRadius: '50%', background: '#00d9ff', display: 'inline-block', boxShadow: '0 0 6px #00d9ff' }} />
                )}
              </button>
              <button
                className={`lang-btn ${lang === 'id' ? 'active-id' : 'inactive'}`}
            ...
            {/* Download */}
            <button
              onClick={() => { window.location.href = `/api/cv/download?lang=${lang}`; }}
              style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '8px 18px', background: lang === 'en' ? '#00d9ff' : '#a78bfa', border: 'none', cursor: 'pointer', color: '#0a0e27', fontFamily: 'DM Mono, monospace', fontSize: 11, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', borderRadius: 4, transition: 'all 0.2s' }}
            >
        ...
        <div style={{ maxWidth: '210mm', margin: '10px auto 0', padding: '0 2rem' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: 'DM Mono, monospace', fontSize: 11, color: '#334155' }}>
            ...
            <span style={{ color: lang === 'en' ? '#00d9ff' : '#a78bfa', fontWeight: 700 }}>
          ...
          <div id="cv-paper" className={animating ? 'fade' : ''}>
                  ...
                  <div key={edu._id} className="cv-entry">
                    <div className="cv-entry-header">
                      ...
                      <span className="cv-entry-date">
                        {formatDate(edu.startDate)}{(edu.startDate || edu.endDate) && ' – '}{edu.endDate ? formatDate(edu.endDate) : L.present}
                      </span>
                    </div>
                    <p className="cv-gpa">{L.gpa}: 3.83/4.00</p>
                    {getAchievements(edu.achievements).length > 0 && (
                      <ul className="cv-bullets">
                        {getAchievements(edu.achievements).map((item, i) => (
                          <li key={i}><span className="cv-bullet-dot">•</span><span>{item}</span></li>
                        ))}
                      </ul>
                    )}
                  </div>
                  ...
                  return (
                    <div key={exp._id} className="cv-entry">
                      <div className="cv-entry-header">
                        ...
                        <span className="cv-entry-date">
                          {formatDate(exp.startDate)} – {isCurrent ? L.present : formatDate(exp.endDate || '')}
                        </span>
                      </div>
                      {achievements.length > 0 ? (
                        ...
                        <ul className="cv-bullets">
                          <li><span className="cv-bullet-dot">•</span><span>{exp.description}</span></li>
                        </ul>
                      ) : null}
                    </div>
                  );
                    ...
                    <div key={project._id} className="cv-entry">
                      ...
                      {project.techStack?.length > 0 && (
                        <p className="cv-body" style={{ marginBottom: '0.25rem' }}>
                          <strong>{L.sections.techStack}:</strong> {project.techStack.join(', ')}
                        </p>
                      )}
            ...
            {skills.length > 0 && (
              <section className="cv-section">
                ...
                {hardSkills.length > 0 && (
                  <p className="cv-body"><strong>{L.sections.techSkills}:</strong> {hardSkills.join(' • ')}</p>
                )}
                {softSkills.length > 0 && (
                  <p className="cv-body"><strong>{L.sections.softSkills}:</strong> {softSkills.join(' • ')}</p>
                )}
              </section>
            )}
